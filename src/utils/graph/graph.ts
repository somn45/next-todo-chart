...
export class Graph {
  constructor(
    ...
    public graphMargin: GraphMargin,
    public dateDomainBase: dateDomainBaseType,
    public texts: string[],
  ...
  // y Axis 설정
  setYAxis = (scale: d3.ScaleLinear<number, number, never>) => {
    if (this.svg) {
      this.svg
        .append("g")
        .attr("class", "yAxis")
...
class GraphRequired {
  constructor(
    ...
    private dateDomainBase: dateDomainBaseType,
...
export class LineGraph extends GraphRequired {
  // 제목 추가
  addTitle = (y: number, width: number, title: string) => {
    if (this.svg) {
      this.svg
