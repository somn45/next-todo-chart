export const GRAPH_WIDTH = 700;
export const GRAPH_HEIGHT = 400;

export const MOBILE_GRAPH_WIDTH = 300;

export const SPARKLINE_WIDTH = 400;
export const SPARKLINE_HEIGHT = 300;

export const DAT_GRAPH_MARGIN = { top: 80, left: 30, bottom: 20, right: 100 };
export const DAT_MOBILE_GRAPH_MARGIN = {
  top: 80,
  left: 60,
  bottom: 20,
  right: 20,
};
export const DAT_LEGEND_TEXTS = ["총합", "할 일", "진행 중", "완료"];
export const DAT_LEGEND_COLORS = ["#000000", "#3498DB", "#FFA500", "#2ECC71"];

export const TL_GRAPH_MARGIN = { top: 80, left: 100, bottom: 20, right: 100 };
export const TL_MOBILE_GRAPH_MARGIN = {
  top: 80,
  left: 60,
  bottom: 20,
  right: 20,
};
export const TL_LEGEND_TEXTS = ["할 일", "진행 중", "완료"];
export const TL_LEGEND_COLORS = ["#3498DB", "#FFA500", "#2ECC71"];

export const GRAPH_LEGEND_PADDING_RIGHT = 60;
export const GRAPH_LEGEND_MARKER_SIZE = { width: 15, height: 2, radius: 5 };
export const DAT_LEGEND_INIT_COORD = { x: 0, y: 0, textX: 22, textY: 6 };
export const TL_LEGEND_INIT_COORD = {
  x: 0,
  y: 0,
  textX: 12,
  textY: 4,
};
